...
Train = True
Finetune = False
train_set = './data/train_val32'
test_set = './data/test_val32'
modelpath = './model'
...
if Train == True:
    ...
    for epoch in range(EPOCH):
        ...
        model.train()
        sum_loss = 0
        for file_name, images, labels in tqdm(train_loader):
            optimizer.zero_grad()  
        ...
        if epoch%1 == 0:
            ...
            model.eval()
            sum_loss = 0
            for file_name, images, labels in test_loader:
                images = images.unsqueeze(1)
                output = model(images)
                # output = torch.sigmoid(output)
                sum_loss += criterion(output, labels.float())
